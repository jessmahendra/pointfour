...
async function detectBrand(tabId, domainOrBrand, url, urlExtraction = null, extractedData = null, pageData = null) {
  // Set a timeout to ensure we always send data back
  const timeoutId = setTimeout(() => {
    console.warn('🎯 Background: detectBrand timeout reached, sending fallback data');
    const timeoutData = {
  ...
  try {
    ...
    if (domainOrBrand.includes('.') || domainOrBrand.includes('://')) {
      ...
      if (!brandPattern) {
        ...
        clearTimeout(timeoutId);
        // Send a message indicating no brand pattern found
        sendBrandDataToTab(tabId, {
          brandName: 'Unknown Brand',
          hasData: false,
          searchType: 'no-pattern',
          recommendation: 'This website doesn\'t appear to be a recognized fashion brand. Try our manual search feature!',
          externalSearchResults: null,
          timestamp: Date.now(),
          error: false
        });
    ...
    try {
      productImage = await extractProductImage(tabId);
    } catch (imageError) {
      console.warn('🖼️ Background: Product image extraction failed:', imageError);
    ...
    // Clear timeout and send data to content script
    clearTimeout(timeoutId);
    ...
    console.log('🎯 Background: Sending error data to tab:', tabId);
...
async function fetchBrandData(brandName, category = 'general', urlExtraction = null, extractedData = null, pageData = null) {
  try {
    ...
    if (urlExtraction) {
      ...
      console.log('🔗 Including URL extraction in API request:', {
...
chrome.runtime.onMessage.addListener((message, sender, sendResponse) => {
  ...
  if (message.type === 'GET_BRAND_DATA') {
    ...
    if (urlExtraction) {
      console.log('🔗 Background: URL extraction data received:', {
        brand: urlExtraction.brand,
        itemName: urlExtraction.itemName,
        confidence: urlExtraction.confidence,
        source: urlExtraction.source
    ...
    console.log('🎯 Background: Calling detectBrand with:', { tabId, brandName, url, urlExtraction });
