...
export default function DirectoryPage() {
  ...
  // Extract unique price ranges and types from brands
  const priceRanges = [
    "All Price Ranges",
    ...Array.from(
      new Set(brands.map((brand) => brand.priceRange).filter(Boolean))
    ),
  ];

  // Determine types from categories (simplified categorization)
  const getType = (category: string) => {
    const lowerCategory = category.toLowerCase();
    if (lowerCategory.includes("footwear") || lowerCategory.includes("shoe"))
      return "Footwear";
    if (lowerCategory.includes("jewelry")) return "Jewelry";
    if (lowerCategory.includes("denim") || lowerCategory.includes("jeans"))
      return "Denim";
    if (
  ...
  const types = [
    "All Types",
    ...Array.from(new Set(brands.map((brand) => getType(brand.category)))),
  ];

  // Filter brands based on search, price range, and type
  const filteredBrands = brands.filter((brand) => {
    const matchesSearch =
      brand.name.toLowerCase().includes(searchTerm.toLowerCase()) ||
      brand.category.toLowerCase().includes(searchTerm.toLowerCase());
    const matchesPriceRange =
      selectedPriceRange === "All Price Ranges" ||
      brand.priceRange === selectedPriceRange;
    const matchesType =
      selectedType === "All Types" || getType(brand.category) === selectedType;
    ...
    return (
      <div className="min-h-screen bg-stone-50 flex items-center justify-center">
        <div className="text-center">
          ...
          <p className="mt-4 text-stone-600">Loading fashion brands...</p>
        </div>
      </div>
    );
  }
    ...
    return (
      <div className="min-h-screen bg-stone-50 flex items-center justify-center">
        <div className="text-center text-red-600">
          <h2 className="text-2xl font-bold mb-4">Error Loading Directory</h2>
          <p className="mb-4">{error}</p>
          <button
            onClick={fetchBrands}
            className="bg-stone-200 text-stone-800 px-6 py-3 rounded-lg hover:bg-stone-300 transition-colors"
          >
            Try Again
          </button>
        </div>
        ...
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
          <div className="text-center">
            ...
            <p className="text-stone-600">
              Discover {brands.length} fashion brands with detailed sizing
              guides
            </p>
          </div>
        </div>
        ...
        <div className="flex flex-col lg:flex-row gap-4 mb-8">
          <div className="relative flex-1">
            ...
            <input
              ...
              className="w-full pl-10 pr-4 py-3 border border-stone-200 rounded-lg focus:ring-2 focus:ring-stone-300 focus:border-transparent bg-white"
            />
          </div>
          <select
            value={selectedPriceRange}
            onChange={(e) => setSelectedPriceRange(e.target.value)}
            className="px-4 py-3 border border-stone-200 rounded-lg focus:ring-2 focus:ring-stone-300 focus:border-transparent lg:w-48 bg-white"
          >
          ...
          {filteredBrands.map((brand) => (
            ...
            >
              <div className="mb-4">
                ...
                {/* Category Tag */}
                <span className="inline-block bg-stone-100 text-stone-700 px-3 py-1 rounded-full text-sm mb-2">
                  {brand.category}
                </span>
                ...
                {/* Fit Notes */}
                {brand.fitNotes && (
                  <p className="text-sm text-stone-700 mb-4 line-clamp-2">
                    {brand.fitNotes}
                  </p>
                )}
              </div>
