...
export async function POST(request: NextRequest) {
  try {
    ...
    console.log('🚀 API (Service Role): Starting enhanced product parsing for:', prompt);
    ...
    console.error('❌ API Error (Service Role):', error);
    return NextResponse.json(
      { 
...
export async function GET(request: NextRequest) {
  try {
    ...
    return NextResponse.json({
      success: true,
      message: 'Enhanced Product Parsing API (Service Role)',
      endpoints: {
